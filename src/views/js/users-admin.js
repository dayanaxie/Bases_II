...
async function loadRealUsers(currentUser) {
    ...
    try {
        ...
        if (result.success && result.users) {
            // Mostrar TODOS los usuarios, incluyendo admins, excepto el usuario actual
            allUsers = result.users.filter(user => user._id !== currentUser._id);
...
function renderUsers(users) {
    ...
    users.forEach(user => {
        ...
        // Dropdown para cambiar tipo de usuario
        const roleDropdown = document.createElement("select");
        roleDropdown.classList.add("role-dropdown");
        roleDropdown.setAttribute("data-user-id", user._id);
        
        roleDropdown.innerHTML = `
            <option value="usuario" ${user.tipoUsuario === 'usuario' ? 'selected' : ''}>Usuario</option>
            <option value="admin" ${user.tipoUsuario === 'admin' ? 'selected' : ''}>Administrador</option>
        `;
        ...
        roleDropdown.addEventListener('change', function() {
            ...
            updateUserRole(userId, newRole, user, this);
        });
...
async function updateUserRole(userId, newRole, user, dropdownElement) {
    try {
        ...
        const token = localStorage.getItem('token');
        const response = await fetch(`http://localhost:3000/api/users/${userId}/tipoUsuario`, {
            method: 'PATCH',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
        ...
        if (result.success) {
            ...
            if (userIndex !== -1) {
                allUsers[userIndex].tipoUsuario = newRole;
            }
...
function performSearch() {
    ...
    if (searchTerm === "") {
        // Si la búsqueda está vacía, mostrar todos los usuarios
        renderUsers(allUsers);
        return;
    }
